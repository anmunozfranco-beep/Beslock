...
Examples:
  ./tools/theme/cache/warm_cloudflare_cache.sh
  ./tools/theme/cache/warm_cloudflare_cache.sh --passes 3
USAGE
}
...
THEME_DIR="$REPO_ROOT/wp-content/themes/beslock-custom"
PRODUCTS_JSON="$THEME_DIR/data/products.json"
MANUALS_DIR="$THEME_DIR/assets/manuals"
...
URLS+=(
  "$BASE_URL/wp-content/themes/beslock-custom/assets/manuals/index.json"
  "$BASE_URL/wp-content/themes/beslock-custom/assets/manuals/export_summary.json"
)

if [ -d "$THEME_DIR/assets/images/hero/clips" ]; then
  ...
  done < <(find "$THEME_DIR/assets/images/hero/clips" -maxdepth 2 -type f \( -name '*.mp4' -o -name '*.webp' \) | sort)
fi

if [ -d "$THEME_DIR/assets/images/hero/overlays" ]; then
  while IFS= read -r hero_image; do
    relative_path="${hero_image#"$THEME_DIR"/}"
    URLS+=("$BASE_URL/wp-content/themes/beslock-custom/$relative_path")
  done < <(find "$THEME_DIR/assets/images/hero/overlays" -type f \( -iname '*.webp' -o -iname '*.png' -o -iname '*.jpg' -o -iname '*.jpeg' \) | sort)
fi

if [ -d "$MANUALS_DIR/products" ]; then
  while IFS= read -r product_json; do
    name="$(basename "$product_json")"
    URLS+=("$BASE_URL/wp-content/themes/beslock-custom/assets/manuals/products/$name")
