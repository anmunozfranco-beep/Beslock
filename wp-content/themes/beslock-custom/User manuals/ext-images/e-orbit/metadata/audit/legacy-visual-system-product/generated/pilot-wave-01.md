...
The pilot scope matches the current dashboard instruction and the first-wave set defined in `../image-generation-matrix.md`.
...
## Execution sequence
1. Read `../../../../ext-images/e-orbit/visual-system/references/e-orbit-visual-profile.md` before generating any slot.
2. Generate 3 variants for each pilot asset outside this folder.
3. Review each variant against `../visual-validation.md` and `../../shared/generation-guides/review-checklist.md`.
...
7. Move slot state to `selected` only after the winner is logged in `selected-assets-register.md`.
...
- Each selected winner passes all product-specific checks in `../visual-validation.md`.
- Each selected winner passes all shared checks in `../../shared/generation-guides/review-checklist.md`.
- `selected-assets-register.md` contains one chosen variant and one stable filename per pilot asset.
- `../image-production-status.md` reflects the actual slot states instead of the initial planning state.
