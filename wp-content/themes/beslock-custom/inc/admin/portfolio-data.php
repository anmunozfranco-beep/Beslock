...
if ( ! function_exists( 'beslock_import_log_dir' ) ) {
  function beslock_import_log_dir() {
    $upload_dir = wp_upload_dir();

    if ( is_array( $upload_dir ) && ! empty( $upload_dir['basedir'] ) ) {
      return trailingslashit( $upload_dir['basedir'] ) . 'beslock_import_logs';
    }

    return trailingslashit( sys_get_temp_dir() ) . 'beslock_import_logs';
  }
}
...
if ( ! function_exists( 'beslock_get_dist_manuals_connectivity_value' ) ) {
  function beslock_get_dist_manuals_connectivity_value( $slug ) {
    ...
    $source_file = trailingslashit( get_stylesheet_directory() ) . 'assets/manuals/products/' . $slug . '.json';
...
if ( ! function_exists( 'beslock_carga_portfolio_process' ) ) {
  function beslock_carga_portfolio_process( $dry_run = false ) {
    ...
    if ( ! $is_dry ) {
      ...
      $log_dir = beslock_import_log_dir();
        if ( ! is_dir( $log_dir ) ) {
